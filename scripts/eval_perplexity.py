...
def load_model(model_path, dtype, base_model_path=None):
    """Load model and tokenizer, auto-detecting PEFT adapters."""
    if _is_peft_adapter(model_path):
        from peft import PeftModel

        # Determine base model from adapter config or explicit arg
...
def main():
    ...
    if args.sft:
        ...
        sft_model, sft_tokenizer = load_model(args.sft, dtype, base_model_path=args.cpt)
