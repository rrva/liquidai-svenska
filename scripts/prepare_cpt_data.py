...
def main():
    ...
    min_chars = args.min_chars if args.min_chars is not None else quality_cfg.get("min_chars", 200)
    max_chars = args.max_chars if args.max_chars is not None else quality_cfg.get("max_chars", 100000)
    min_words = args.min_words if args.min_words is not None else quality_cfg.get("min_words", 30)
    eval_ratio = args.eval_ratio if args.eval_ratio is not None else config.get("splits", {}).get("cpt_eval_ratio", 0.01)
